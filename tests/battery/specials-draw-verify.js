...
// #121: the six shipped specials + the three Citadel additions (leap/castvolley/raisecourt) + S3 smite
// + S4 kegburst (Emberkeg radial burst) + webvolley (Broodmother web spread).
const NAMES = ['slam', 'charge', 'nova', 'summon', 'pullunder', 'raiseadds', 'leap', 'castvolley', 'raisecourt', 'smite', 'kegburst', 'webvolley'];

// ---- §1 registry shape -------------------------------------------------------------------
ok('1a. CONTENT.specials holds exactly the twelve special keys, in order', !!C && JSON.stringify(Object.keys(C.specials || {})) === JSON.stringify(NAMES), C && JSON.stringify(Object.keys(C.specials || {})));
ok(
...
ok(
  '1c. windups are the shipped values (slam46/charge32/nova34/summon40/pullunder48/raiseadds44 · leap40/castvolley30/raisecourt44 · smite52 · kegburst70/webvolley30)',
  !!C && [46, 32, 34, 40, 48, 44, 40, 30, 44, 52, 70, 30].every((w, i) => C.specials[NAMES[i]].wind === w),
...
const EXPECT = { slam: 6, charge: 4, nova: 3, summon: 3, pullunder: 24, raiseadds: 15, leap: 6, castvolley: 36, raisecourt: 15, smite: 16, kegburst: 34, webvolley: 12 };
