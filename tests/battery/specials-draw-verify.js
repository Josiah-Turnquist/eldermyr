...
const NAMES = ['slam', 'charge', 'nova', 'summon', 'pullunder', 'raiseadds', 'leap', 'castvolley', 'raisecourt', 'smite'];

// ---- §1 registry shape -------------------------------------------------------------------
ok('1a. CONTENT.specials holds exactly the ten special keys, in order', !!C && JSON.stringify(Object.keys(C.specials || {})) === JSON.stringify(NAMES), C && JSON.stringify(Object.keys(C.specials || {})));
ok(
...
ok(
  '1c. windups are the shipped values (slam46/charge32/nova34/summon40/pullunder48/raiseadds44 · leap40/castvolley30/raisecourt44 · smite52)',
  !!C && [46, 32, 34, 40, 48, 44, 40, 30, 44, 52].every((w, i) => C.specials[NAMES[i]].wind === w),
...
const EXPECT = { slam: 6, charge: 4, nova: 3, summon: 3, pullunder: 24, raiseadds: 15, leap: 6, castvolley: 36, raisecourt: 15, smite: 16 };
