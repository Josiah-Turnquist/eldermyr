...
feed (`_feedSeen`), quest state (`_qSeen`), the Dread Legion roster (`_lgSeen` vs `_lgN`), and
the dungeon tile grid (`_sentDgN` vs `_mapSwitchN`). **dgTiles is transactional:** the flag is
