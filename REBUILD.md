...
## the one-minute automated gate)

- **TIER 1 — data/platform slices** (table moves, registry migrations, refactors with no
  intended behavior change): Opus-tier agents, batched 2-3 slices per agent. Gates =
  battery + golden check + mp check + typecheck + build (≈1 min, automated — the oracles
  ARE the safety net; a moved hash fails the gate). New assertions get an inline perturb
  spot-check (seconds). NO scratch-worktree vacuity rituals, NO per-file type-error
  demos, NO browser smoke unless the change is visible.
- **TIER 2 — behavior slices** (features, balance, anything a player feels): Opus by
  default; seen-to-fail proofs for NEW gameplay assertions, conscious-re-record protocol
  with divergence evidence when hashes legitimately move, live smoke for player-visible
  surfaces. Lean reports.
- **Fable-tier agents**: reserved for genuine judgment hazards (quest-aliasing-class
  problems), not mechanical migration.
...
- 2026-07-16: P3/S2 DONE — enemy kinds (registry #2 + #3): makeEnemy's 8-kind base table + per-type init blocks (p03:201-318) → `src/content/enemies.ts` ENEMIES entries (data + `init(e)` hooks — verbatim bodies, SAME Math.random draws, wobble still the FIRST draw), the makeWildEnemy ring/biome ternary chains (p03:353-376) → WILD_SPAWN threshold tables + a `pick(r,table)` walk with the EXACT old constants/order/strict-< (deviation, documented in types.ts: per-kind normalized weights would re-map r-space + can't encode per-ring order — that shape upgrade is a conscious re-record for a feature slice, not a migration; ring/level BRANCHING and scaling curves stay in-part until S11), and makeDungeonEnemy's archer@3/healer@4 growth (p03:392-397) → `src/content/dungeons.ts` poolGrowth rows (row order = old if-order). p03 consumes via CONTENT.enemies[type]/CONTENT.wildSpawn/CONTENT.dungeons at the factory sites (no top-level binding existed to alias — plan §1.3's dispatch-site form); only primitives are copied off shared rows, so no registry object can reach live sim data. Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, identity by both runs, pre+post legs), battery 46/46 (one v4b wall-clock flake in run 1 — its documented class; solo + full-rerun green), world self-test + typecheck green (deliberate type errors per new file SEEN failing: enemies.ts hp:'11' TS2322, dungeons.ts add:'lich' TS2322 vs EnemyKindKey — reverted), build idempotent from rm -rf dist ×2 (a52c56ba…). content-purity 17→41 asserts: §5e-5g shape/value pins, §6 canary now FUNCTION-AWARE (hooks serialize as source — the S2 header-note extension) + §6b sees-hooks guard, NEW §7 child process (facing-noregress CAPTURE-patch pattern: makeEnemy/makeDungeonEnemy) probing raw rows, exact forced-draw init values (castCd 80/attackCd 50/chargeCd 100/healCd 120), draw ORDER, vale/lava/frozen walks incl. the 0.55 strict-< boundary, dungeon knob@2/3/4, and runtime perturb→restore THROUGH-ness ×4. Vacuity vs three scratch trees (never the repo): value-perturbed (slime hp 12) → 5f/7a/7h fail; p03-reverted-to-inline (registry shipped but unread) → 7h/7i/7p/7t fail; updateWeather swapping mage.init mid-run → 6a fails under the fn-aware projection while the OLD plain-JSON projection stayed green (blindness demo'd live — the extension's teeth). Live browser smoke on the local MP server: page boots off the S2 artifact with window.CONTENT carrying all four keys, 16 Vale foes on the wire holding registry row values (slime name/w20/#60d060), browser hero took real contact damage, and a second REAL-socket probe hero walked 1418px east on held keys then killed 2 registry-built foes (xp 0→27, gold 0→18; 30 foes in AOI, kinds slime/bat/skeleton = the Vale tables exactly) — 0 console errors throughout. S3 (creature art → entry.draw + faces) is next.
- 2026-07-16: P3/S3 DONE — creature art: drawEnemy's 11 branches (~940 lines, p20) → `draw(v, e)` hooks on the enemies.ts entries VERBATIM (script-extracted, reverse-transform byte-proven; only change: `ctx.` → the DrawView arg {g2d,sx,sy,flash,shade,rgbOf} — flash rides verbatim as boolean|null); boss/dragon/kraken join as draw-hook-ONLY entries (stats stay factory until S5); FACING → per-entry `faces:true` with CONTENT.facing DERIVED + faceDz, p20 keeps positional aliases (p17 byte-unchanged); dispatch + prelude + whole postlude stay in-part, one reusable _DV per call. facing-noregress EXTENDED to ALL 11 + steed = the op-for-op oracle: HEAD-inline vs WT-hooks IDENTICAL (402 ops) — seen failing vs a kraken-op-perturbed scratch tree. Gates: golden 8/8 + mp 4/4 byte-untouched oracles (24c2ed95…/0db3e62a…), battery 46/46, self-test + typecheck green (draw-is-mandatory TS2561 / readonly-e TS2540 / outside-Draw2D TS2339 each seen failing, reverted), build idempotent ×2 (6ba90430…), content-purity 41→47 (11-kinds pin, art-only trio, derived facing, draw-source canary token, §7 dispatch/DrawView/facing through-ness — seen failing vs pre-S3 HEAD-6dfa596 tree). Live smoke (visible slice): Vale slime/bats canonical colors, war-camp RED hound/skeleton + ★ELITE (e.color derivation through hooks), hound TURNED live (_faceL 1→0, mirror on screen), Emberwyrm faces the hero with tag unmirrored — 0 console errors. S4 (boss specials) next; standing perf-review pass over the render slice = integrator follow-up.
- 2026-07-16: P3/S4 DONE — boss specials registry: the telegraph triad (DESIGN silent-failure #1) is now one entry per special in `src/content/specials.ts` — `wind` (windup, was startBossSpecial's inline `{slam:46,…}[name]` table), `exec` (effect, the execBossSpecial branch p17:407+), `drawTele` (telegraph, the p20 `nm===…` chain). exec/drawTele are VERBATIM hook moves (the S3 discipline): drawTele paints through the `v.g2d` DrawView; the 6 impure exec branches (Sound/state/factory reach-ins) destructure a curated non-banned SpecialActView bag built at the p17 dispatch (`sfx`=Sound, `enemies`/`player`/`map`/`dungeonLevel`=state slices, live refs so pushes/writes land on the real world) — the ONLY textual change is `Sound.`→`sfx.` and `state.X`→the destructured `X`, same op order + same Math.random draws (summon/raiseadds). p17 keeps thin wrappers: bossSpecials slices CONTENT.specialRoster (base copied — risk #3), startBossSpecial reads `.wind` (`||36` fallback bit-preserved), execBossSpecial dispatches `.exec`; p20 dispatches `.drawTele(_DV,e)`. `drawTele` is REQUIRED on the Special interface (a special with no telegraph = a type error). None of the boss-special fns are CAPTURE/NAMES (internal). Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, identity by both runs — verbatim move, zero hashes moved), typecheck clean, world self-test green, build clean, battery 46→47 with NEW `specials-draw-verify` (19 asserts: registry shape/windups/roster, windup+roster read THROUGH the registry incl. poke→restore, per-name drawTele op-counts via a counting g2d [slam6/charge4/nova3/summon3/pullunder24/raiseadds15], dispatch source guards + inline-branches-gone) — SEEN FAILING vs an emptied slam.drawTele scratch (rebuilt: §3.slam 6→0). facing-noregress unaffected (it sets e.tele=null → telegraph skipped both sides). S5 (apex registry — the hunt tripwire) next.
- 2026-07-16: P3/S5 DONE — apex-boss registry (the #10 fusion split, DATA half): the world-boss DATA moved to `src/content/apex.ts` — the 4 Great Hunts (GREAT_HUNTS), 2 pinnacles (PINNACLE_BOSSES), and the dragon/pin tuning consts (DRAGON_LEVEL/DRAGON_COLOR, PIN_LEVEL, PIN_ARENA_START/MIN/SHRINK/LEASH) VERBATIM, incl. each hunt's fixed-trophy reward + each pinnacle's unique-drop descriptor. p22 keeps a POSITIONAL ALIAS at every old declaration line (`const GREAT_HUNTS = CONTENT.apex.hunts;` etc. — the mission's stated method / plan §1.3's worked form), so all 9 consumers (p06/p09/p12/p14/p17/p21/p22 + world.js `G.GREAT_HUNTS`), the CAPTURE'd namespace (GREAT_HUNTS + PINNACLE_BOSSES are grabbed by the server), and the RNG draw order are UNTOUCHED — the scaling factories (makeGreatBeast/makePinnacleBoss/makeWildDragon/makeKraken), pinnacleLair, makePinnacleAdd, the RNG drop rollers (dropGreatBeastReward/dropPinnacleReward) and the respawn machinery all stay in-part reading the same values at the same program position. DELIBERATE DEFERRAL (documented in apex.ts + here): the stats/lair/adds/drops HOOK extraction (plan §1.2) and the respawn descriptors are NOT done — that's the higher-risk step (it threads state/partyN/partyLvl/distFactor through ctx bags and the drop rollers ride the hunt-perturb control), better as its own slice; this slice is the hash-safe data move the mission's method describes. Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, identity by both runs — verbatim data, zero hashes moved), typecheck clean, world self-test green, build clean. THE S5 TRIPWIRE: `prove` hunt control still diverges EXACTLY @700 — and the perturb (GREAT_HUNTS hp/atk ×1.5 via the namespace) reaches makeGreatBeast THROUGH the registry alias, so it doubles as a live pipe-through proof (4 beasts respawn with ×1.5 stats, enemyHp 90067→93605 @700, samples 0-6 identical). Battery 47/47; content-purity 47→51 asserts (§5j-5m: apex shape/shipped values incl. Titan’s Maul + the CAPTURE'd-alias identity NS.GREAT_HUNTS===C.apex.hunts; §6 canary now covers apex — great beasts spawn at setupOverworld so `e.specials=h.specials` is live in the 3k-tick run, proven read-only) — SEEN FAILING vs a scratch (frosttitan hp 900→901 → §5k). S6 (gear registry) next.
- 2026-07-16: P3/S6 DONE — gear registry: the loot/shop/mastery DATA → `src/content/gear.ts` (byte-exact, script-extracted from the literals): RARITIES/STYLE_NAMES/ARMOR_NAMES/RAR_PREFIX (p01), MASTERY/MASTERY_LVLS/PATTERN_WEAPONS (p02), SHOP_NAMES/SHOP_WEAPONS (p08, the 13-weapon table), SHOP_ARMOR (p09), UNIQUES (p22, the 4 relics), plus the generator's style/element pools and rollAffixes' offense/defense pool DEFINITIONS as a pure `affixPool(rIdx,isWeapon)` helper (NO RNG — the pick + second-affix roll stay in rollAffixes, so the draw order is byte-identical). All class-P: every part keeps a POSITIONAL ALIAS at the old declaration line (`const RARITIES = CONTENT.gear.rarities;` etc.; genWeapon reads CONTENT.gear.genStyles/genElements, rollAffixes reads CONTENT.gear.affixPool at the dispatch sites) and the loot factories (genWeapon/genArmor/makeUnique/genShopStock/rollPatternWeapon) consume them unchanged. None of these symbols are CAPTURE'd or read externally (pure in-part convenience). Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, identity by both runs — the affixPool + gen-pool moves preserved RNG order, zero hashes moved), typecheck clean, world self-test green, build clean. Battery 47/47; content-purity 51→54 (§5n-5p: gear table counts + shipped values [legendary ×2.05, Steel Sword 80g, mastery 10/18/24, leviathanspine ×1.2] + affixPool rIdx-scaled behavior; §6 canary covers gear — genWeapon/rollAffixes/rollPatternWeapon read+deep-copy, never mutate the registry, proven over 3k ticks; no namespace-identity assert since the symbols aren't CAPTURE'd — golden's byte-identical loot IS the live through-ness proof) — SEEN FAILING vs a scratch (Steel Sword 80→81 → §5o). S7 (elite affixes) next.
- 2026-07-16: P3/S7 DONE — elite-affix registry: AFX_DEFS + AFX_KEYS (the 4 "variety at zero art cost" enemy affixes, p18) → `src/content/affixes.ts` as DATA (flag/label/pre per key + the ordered key pool), and the per-key SEEDING rollEliteAffixes ran inline (p18:50-57, the `if (key==='shielded')…else if (key==='warded')…` block) → per-affix `apply(e)` HOOKS (pure — they mutate ONLY the enemy: shielded stamps a 25%-maxHp shield pool, warded arms its ward window; vampiric/splitting omit apply). The affix PICK (the RNG splice) stays in rollEliteAffixes and the HIT behaviour stays in the hot-path afxHit — data-driven, not a hook-per-hit (p18's own note). p18 keeps positional aliases (`const AFX_DEFS = CONTENT.affixes.defs;`); rollEliteAffixes calls `if (d.apply) d.apply(e)` in place of the branch (byte-identical, no RNG change). AFX_DEFS/AFX_KEYS not CAPTURE'd; rollEliteAffixes/afxHit ARE (unchanged, keep their bindings). Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, identity by both runs — no hashes moved), typecheck clean, world self-test green, build idempotent (f70cb9e8… ×2 from rm -rf dist — the batch-level check). Battery 47/47 (one v4b-fullstack wall-clock flake under concurrent load; PASS solo on retry — its documented class); affix-verify 75/75 confirms a shielded elite seeds its shield pool THROUGH the apply hook + serializes (elite affixes fire only at partyLvl>=15, so golden windows never exercise them). content-purity 54→57 (§5r-5t: affix shape/flags/labels + the apply-hook SEEDING behavior — shielded→25% pool, warded→ward window, vampiric/splitting none; the sole guard for the extracted apply logic golden can't see; §6 canary covers affixes) — SEEN FAILING vs a scratch (shielded.apply 0.25→0.30 → §5t). P3/S4-S7 batch COMPLETE.
- 2026-07-16: P3/S8 DONE — dungeon registry (Tier-1 batch): "Dungeon 2.0" DATA → `src/content/dungeons.ts` beside S2's poolGrowth — DUNGEON_THEMES (4 themes, VERBATIM) + `theme(level)` depth index, FLOOR_MODS (4 mods) + the rollFloorMod weight ladder as an RNG-FREE `pickFloorMod(r)`, and the Key-Vault knobs `{minLevel:2, odds:0.4}`. The RNG DRAWS STAY IN-PART (the S2 pick(r,table) discipline): rollFloorMod keeps `if(level<2)return null` + its single `Math.random()` then maps via pickFloorMod; generateDungeon keeps its vault draw and reads vault.minLevel/odds — draw count/order byte-identical. p03/p06 keep POSITIONAL ALIASES at the old decl lines (`const DUNGEON_THEMES = CONTENT.dungeons.themes;` read by p18 drawDungeon [0]; `const FLOOR_MODS = CONTENT.dungeons.floorMods;` read by p22 HUD + p06 log; `dungeonTheme(level)` wraps `theme()` for p10/p11). state.dungeonThemeData still holds a theme row by reference — verified NO mutation site (drawDungeon/generateDungeon/makeDungeonEnemy read theme.key/.pool/.floor only; .pool.slice() copies), so serialize-by-value + snap.dgTheme safeClone hash identically. None CAPTURE'd (only world.js COMMENTs mention COMP_CAP/dungeonThemeData). Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, pure data move, zero hashes moved), typecheck clean, world self-test green, build clean. content-purity 57→60 (§5u-5w: themes/floormods/vault shape+values, theme(level) depth-index VERBATIM, pickFloorMod strict-`<` ladder incl. the 0.55 boundary) — inline perturb spot-check: vault odds 0.4→0.41 + inferno pitKind lava→pit + pickFloorMod 0.55→0.56 each tripped 5u/5v/5w, restored 60/60. S9 (companions) next.
- 2026-07-16: P3/S9 DONE — companion registry (Tier-1 batch): the 3-class warband DATA + level-scaling → `src/content/companions.ts` — COMP_CLASSES (knight/ranger/mage VERBATIM), COMP_NAMES (16), COMP_CAP (3), and compStatsFor's formula as `statsFor(cls, level, tier=0)`. TIERS-READY for #115/F2: each class carries `tiers:[{statMul:1}]` (tiers[0]=T1=today) and statsFor applies `tiers[tier].statMul`; byte-identical at tier 0 because `x*1===x` for every finite double, so `Math.round(baseHp*f*1)` equals the old `Math.round(baseHp*f)` to the bit (F2 appends T2/T3 + hire/upkeep as one row each). p04 keeps POSITIONAL ALIASES at the old decl lines (`const COMP_CLASSES = CONTENT.companions.classes;` read by p04/p05/p07; COMP_NAMES by p04; COMP_CAP by p04/p05; `compStatsFor(cls,level)` wraps statsFor for p04). Class rows keep top-level `hire` (p04 reads COMP_CLASSES[cls].hire). None CAPTURE'd (world.js only mentions COMP_CAP in a comment); no mutation site (instances copy primitives). Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, zero hashes moved — golden windows contain NO companions per the P2/S19 note, so statsFor is oracle-invisible), typecheck clean, world self-test green, build clean. content-purity 60→62 (§5x-5y: class/name/cap shape+values + tiers[0].statMul, and statsFor's VERBATIM formula at knight L1/L5/L10 + mage L1 — the sole guard for the oracle-invisible formula) — inline perturb: knight baseHp 120→121 tripped 5x AND 5y (statsFor L10 271→273, proving through-ness), restored 62/62. S10 (small-tables sweep) next.
- 2026-07-16: P3/S10 DONE — small-tables sweep (Tier-1 batch): 22 display/flavor DATA tables across 9 parts → `src/content/tables.ts` (CONTENT.tables) as 11 clusters — seasons (SEASONS/SEASON_TINT/SEASON_ICON, p14; SEASON_LEN stays in-part as a formula knob), foods (INGR/FOODS/FOOD_LABEL p14 + FORAGE_VALUE p09), bless (p10), trade (TRADE_GOODS p09), status (STATUS_DESC p15), regions (REGION_NAMES p15 + REGION_SUBS/LORE_TEXTS p05), poi (POI_KINDS p03), holds (HOLD_SITES p04), warlord (WL_FIRST/WL_EPITHET/RANK_NAMES/WL_STRENGTHS/WL_WEAKNESS p15 — NAMING/label only; the strength EFFECTS + warlord stat curve stay in makeWarlordEnemy per the plan), nemesis (NEMESIS_NAMES/NEMESIS_TITLES p13), abilities (ABILITY_RMAX p08 + HEAT_AURA_MIN/TICKS p11). Every part keeps a POSITIONAL ALIAS at the old decl line; NONE captured by server/client (all in-game display). Byte-identity of the hand-typed registry vs the part sources PROVEN 25/25 by a scratch extract-and-deep-compare (all unicode intact — U+2212 −, U+2014 —, curly quotes). STEED: its colour/level already live in apex.ts (S5 — ONE source), so no steed row here; the drawPlayer mounted-steed HOOK (~175 lines coupled to drawPlayer's sx/sy/bob frame, animated by Date.now()) is the S5-style H-half, DEFERRED (facing-noregress already guards it op-for-op when it lands). Entanglement check (the STOP condition) CLEARED: HOLD_SITES→state.holdings is `.map()`ed to FRESH objects (rows never aliased into state); WL_STRENGTHS/WL_WEAKNESS are description strings, not effect logic. Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…, zero hashes moved — the oracle-REACHING tables [WL names→e.name, TRADE.base/FORAGE→gold, HOLDS coords, ABILITY_RMAX→ranks, HEAT_AURA→aura state] proven byte-identical by golden itself), typecheck clean, world self-test green, facing-noregress green (steed + all 11 creatures pixel-identical — the aliases feed identical values into draw), build clean, battery 47/47. content-purity 62→65 (§5z1-5z3: 11-cluster shape/counts, oracle-reaching values, and the DISPLAY-ONLY unicode byte-for-byte — the guard for what golden can't see) — inline perturb: WL_FIRST 18→17 + furs base 40→41 + ward desc U+2212→hyphen each tripped 5z1/5z2/5z3, restored 65/65. REPOINT (textual-probe class, the S1-S3 precedent): map-enlarge-verify extracted `const HOLD_SITES = […]` from the artifact TEXT — that literal moved to the registry, so it now reads globalThis.CONTENT.tables.holds (the real values the artifact eval populates; §5z2 pins them). S11 (curves) next.
- 2026-07-16: P3/S11 DONE — curves registry (Tier-1 batch; the CAUTION-#1 formula slice): the level/distance SCALING FORMULAS → `src/content/curves.ts` — xpForLevel (p12), the makeWildEnemy stat factor `wildStat` + reward factor `wildReward`, makeDungeonEnemy `dungeonStat` + the grind-premium knobs (dungeonXpMul 1.4 / dungeonGoldMul 1.25), the shared `ascMul` (1 + ascension*0.2, ONE source for dungeon enemy AND boss), and the dungeon-boss level factor `dungeonBossStat`. EXTRACTION ONLY — identical math, identical operator order, identical rounding: `Math.round` stays at every CALL SITE (the factory), only the raw FACTOR expression moved, and the diffMul(df)/biomeMul draws + the type-roll/pool-pick RNG stay in-part. Content can't read state, so ascension/df/level ride in as ARGS. p03/p12 keep positional-alias forms at the old sites (`function xpForLevel(L){return CONTENT.curves.xpForLevel(L);}`; makeWildEnemy reads `CONTENT.curves.wildStat/wildReward`, makeDungeonEnemy `dungeonStat/*Mul`, makeDungeonBoss `ascMul/dungeonBossStat` with `asc` kept a local for its atk; base stat literals 90/12+level*2.2/4+level/100/200 stay in the factory). None CAPTURE'd (xpForLevel is p12-only; makeWildEnemy the CAPTURE is unchanged). Gates: golden 1p 8/8 + mp 4/4 on BYTE-UNTOUCHED oracles (24c2ed95…/0db3e62a…) — THE FLOAT-IDENTITY PROOF: every trajectory spawns+levels through these fns, so byte-identity == call-order/float-identical (no hash moved → no STOP). typecheck clean, world self-test green, build IDEMPOTENT (d9c376ac… from rm -rf dist ×2 — the batch-level check). content-purity 65→67 (§5z4-5z5: xpForLevel VERBATIM at L1/L2/L7/L10 = 32/55/481/1924, the xp/gold/ascension knobs, and the wild/dungeon/boss factor formulas at sample points) — inline perturb: base 22→23 + wildReward df²·1.3→1.4 + xpMul 1.4→1.5 tripped 5z4/5z5 AND drove golden to DIVERGE at tick 0 (proving the curves are reached LIVE, not dead extraction), restored 67/67 + golden 8/8. This STAGES F1 (#113 wildReward level-term, a designed re-record — NOT this slice). P3/S8-S11 batch COMPLETE.
- 2026-07-16: P3/F1 DONE — #113 OVERWORLD REWARD CURVE (the ONE designed re-record for tuning): the flat
  `wildReward` (biome+distance only — a L30 hero fighting a stat-scaled Frontier foe was paid L1 gold) SPLIT
  into `wildXp`/`wildGold` in `src/content/curves.ts`, each gaining a level term — XP the FULL wild-stat slope
  `(1+(lvl-1)*0.26)`, gold a GENTLER `(1+(lvl-1)*0.10)` (gold has other faucets). At L1 both terms are ×1, so
  the curve is byte-identical to pre-F1 below L2. Files: curves.ts (+ CurveRegistry in types.ts), p03
  makeWildEnemy call site (threads `lvl` in scope + splits the two Math.round reward lines — the ONE call-site
  edit "curves.ts only" couldn't avoid, since xp/gold now diverge by level), content-purity §5z5 fixture. Reward
  table (Skeleton xp18/gold12, Frontier df=1): L1 59xp/40g→UNCHANGED; L5 ×2.04xp/×1.40g; L15 ×4.64/×2.40; L30
  ×8.54/×3.90. DIVERGENCE EVIDENCE (old dumps captured pre-edit, mirror proven byte-equal to both oracles): the
  1p oracle is BYTE-UNCHANGED (24c2ed95… — check 8/8 after the edit; NO 1p golden trajectory spawns a wild foe
  while ≥L2 in-window, so the term never fires there — re-recording it produced the identical file); oracle-mp.json
  RE-RECORDED (0db3e62a…→d82009cf…) — mp-overworld-combat diverges seed1337@tick600 / seed98765@tick500, and the
  leaf-diff of the state at the first divergent sample is EXACTLY 2 leaves — ONE wild enemy's `xp 11→16` + `gold
  6→8` (both UP, before any level-up cascade), nothing else (risk #7 clear); mp-day-rollover byte-identical, so
  the 51 moved hashes are precisely the post-divergence tails of the two combat trajectories (25+26). Gates: full
  prove + mp-prove GREEN on the new baselines — determinism (byte-identical cross-process streams), speed cascade
  @0, and the day-boundary HUNT control still diverges EXACTLY @700 (samples 0-6 identical — the respawn path
  untouched). typecheck clean, world self-test green, battery 47/47 (§5z5 rewritten to pin the split curves + the
  XP>gold slope divergence — SEEN FAILING on a 0.26→0.27 source perturb, restored 67/67 internal). GAME_VERSION
  unbumped (ships at v3.0.0 cutover). Deviation: p03 call site touched (unavoidable — see above).
- 2026-07-16: P3/F2 DONE — #115 WARBAND ECONOMY (oracles prove-identical path): hireables are now 3 TIERS × 3
  classes. `src/content/companions.ts` — each class carries `tiers:[T1,T2,T3]` (tiers[0]=T1=today EXACTLY, statMul
  1 → byte-identical, `x*1===x`); ladders `statMul 1/2/4`, `upkeep 10/60/300 g/day per head`, `hire` = base ×10^tier
  (knight/ranger 200/2000/20000, mage 240/2400/24000); CompanionTier gains hire/upkeep (types.ts); top-level
  class.hire kept (§5x pin). Machinery (p04/p05): compStatsFor/makeCompanion/recruitCompanion(cls,tier)/
  bumpCompanionLevels thread the tier (c.tier rides the instance); NEW `companionUpkeep()` in **onNewDayHero**
  (p13) charges the acting hero for HIS heads only (`c.ownerId===state.player.id`; SP: both unset →
  undefined===undefined → all his; the party()/tribute precedent — upkeep is not positional). SHORT of the full
  bill → NOBODY paid, every one of his heads flips `c.unpaid=1`; paid in full next day-tick → cleared. UNPAID =
  the refusal gate `&& !c.unpaid` on updateCompanionsFor's engage block — the companion STAYS + FOLLOWS but won't
  chase or swing (the owner's exact rule). renderCompanions shows 3 tier buttons/costs/upkeep + the roster's
  tier tag + "✊ unpaid — won't fight". Serialization: `tier`+`unpaid` added to SP save (p06)/load (p07) AND MP
  save+rejoin (world.js) whitelists; both ride packComp/safeClone (scalars) to the wire free. NO RPC allow-list
  change (recruitCompanion already whitelisted; the [cls,tier] args ride `_runRpc`; mp.html reuses window.
  renderCompanions + forwards recruitCompanion). ORACLES UNMOVED — golden 8/8 + mp 4/4 by BOTH-RUNS IDENTITY on
  the freshly-re-recorded post-F1 baselines (no companions in the golden windows; companionUpkeep no-ops on the
  empty roster with no RNG/log, so the day-700 tick is byte-identical). NEW battery `warband-econ-verify` (26
  asserts: tier hire/upkeep/statMul tables, T1 byte-identity incl. tier-less-save→T1, RPC recruit stamps
  tier+ownerId at the right cost, per-HEAD/per-OWNER upkeep via the real day boundary [A charged 70, B charged 10,
  zero cross-bleed], insufficient→unpaid+gold-not-deducted, the REFUSAL GATE [adjacent foe takes ZERO damage] —
  SEEN FAILING by deleting `&& !c.unpaid` [unpaid comp then swings, F1 flips FAIL], PAID resumes, day-tick clears
  unpaid+deducts, snapshot carries tier/unpaid) → battery 48/48. LIVE SMOKE (SP page off the built dist): the hire
  panel renders 9 tier buttons with correct costs + "upkeep 10/60/300 g/day" lines; a hired T2 Knight shows
  "Knight · T2 · Lv 8 · ⚔44 · 60g/day"; the unpaid marker renders; and with an adjacent foe the UNPAID knight left
